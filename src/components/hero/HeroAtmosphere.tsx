...
import { useMediaQuery, useMousePosition } from "@/hooks/useMotion";
import { EASE_OUT_EXPO } from "@/lib/motion";

export function HeroAtmosphere() {
  const { x, y } = useMousePosition();
  const reduced = useReducedMotion();
  // Default true so first paint on phones skips giant CSS blurs.
  ...
  return (
    <div className="absolute inset-0 -z-0 overflow-hidden bg-bg" aria-hidden>
      ...
      {isMobile ? (
        <>
          <div className="pointer-events-none absolute -right-[20%] top-[8%] h-[42vh] w-[42vh] rounded-full bg-accent/[0.09]" />
          <div className="pointer-events-none absolute -left-[18%] bottom-[12%] h-[32vh] w-[32vh] rounded-full bg-highlight/[0.08]" />
        </>
      ) : (
        <>
          <motion.div
